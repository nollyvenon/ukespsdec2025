            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="flex justify-between items-center mb-6">
                        <h1 class="text-2xl font-bold">Manage Job Listings</h1>
                    </div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse($jobListings as $job)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-500">{{ $job->poster->name ?? 'N/A' }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-500">{{ ucfirst(str_replace('_', ' ', $job->job_type)) }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                                {{ $job->job_status === 'published' ? 'bg-green-100 text-green-800' : 
                                                   ($job->job_status === 'draft' ? 'bg-yellow-100 text-yellow-800' : 
