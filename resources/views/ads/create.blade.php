            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('admin.ads.store') }}" enctype="multipart/form-data">
                        ...
                        <div class="mt-6">
                            <label for="ad_campaign_id" class="block text-sm font-medium text-gray-700">Campaign (Optional)</label>
                            <select name="ad_campaign_id" id="ad_campaign_id"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                <option value="">No Campaign</option>
                                @foreach($campaigns as $campaign)
                                    <option value="{{ $campaign->id }}" {{ old('ad_campaign_id') == $campaign->id ? 'selected' : '' }}>
                                        {{ $campaign->name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('ad_campaign_id')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                            <div>
                                ...
                                <select name="position" id="position"
                                    ...
                                    <option value="below_header" {{ old('position') === 'below_header' ? 'selected' : '' }}>Below Header</option>
                                    <option value="above_footer" {{ old('position') === 'above_footer' ? 'selected' : '' }}>Above Footer</option>
                                    <option value="left_sidebar" {{ old('position') === 'left_sidebar' ? 'selected' : '' }}>Left Sidebar</option>
                                    <option value="right_sidebar" {{ old('position') === 'right_sidebar' ? 'selected' : '' }}>Right Sidebar</option>
                                    <option value="top_slider" {{ old('position') === 'top_slider' ? 'selected' : '' }}>Homepage Slider</option>
                                </select>
                                @error('position')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                        ...
                        <div class="mt-8 p-4 bg-gray-50 rounded-lg">
                            ...
                            <div class="mt-4">
                                ...
                                <textarea name="slider_description" id="slider_description" rows="3"
                                          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('slider_description') }}</textarea>
                            ...
                            <div class="mt-4">
                                <label for="is_slider_featured" class="flex items-center">
                                    <input type="checkbox" name="is_slider_featured" id="is_slider_featured" 
                                           value="1" {{ old('is_slider_featured') ? 'checked' : '' }}
